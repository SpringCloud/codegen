        ...
        <div class="col-sm-8 col-sm-offset-2">
          <div class="ibox float-e-margins">
            <div class="ibox-content">
              <div id="genform" class="form-horizontal">
                <div class="module-item" v-for="(module,number) in modules" :key="number">
                  ...
                  <transition name="fade" v-else-if="module.type == 'RADIO_GROUP' && index == 1 && module.key == 'sc-alone-radio' && showScAloneRadio">
                    <div class="form-group">
                      ...
                      <div class="col-sm-10">
                        <div v-for="(radio,rnum) in module.entityList" class="col-sm-2" v-if="index == 1">
                          <el-radio  :label="radio.key" v-model="Independent" :name="module.key">{{ radio.label }}</el-radio>
                        </div>
                      </div>
                    </div>
                  </transition>
                    ...
                    <div class="form-group" >
                      ...
                      <div class="col-sm-10">
                        <div v-for="(radio,rnum) in module.entityList" class="col-sm-2" v-if="index == 1">
                          ...
                          <el-radio  :label="radio.key" v-model="dorckerType" :name="module.key" >{{ radio.label }}</el-radio>
                        </div>
                      </div>
                    </div>
                  </transition>
                  <transition name="fade" v-else-if="module.type == 'CHECKBOX_GROUP' && index == 1 && module.key == 'sc-group-checkBox' && showScGroupCheckBox">
                    <div class="form-group">
                      ...
                      <div class="col-sm-10" v-if="index == 1">
                        <!-- <el-checkbox-group v-model="basicFramework">
                          ...
                          <el-checkbox-group v-model="basicFramework">
                            <el-checkbox v-for="chkbox in module.entityList" :class="'col-sm-2'" :label="chkbox.key" :key="chkbox.key">{{chkbox.defaultable == true?chkbox.label+'(推荐)': chkbox.label }}</el-checkbox>
                          </el-checkbox-group>
                        <!-- </el-checkbox-group> -->
                      </div>
                    </div>
                    ...
                    <div class="form-group">
                      ...
                      <div class="col-sm-10" v-if="index == 1">
                        <!-- <div v-for="(chkbox,cnum) in module.entityList" :class="'col-sm-2'">
                          ...
                          <el-checkbox-group v-model="basicFramework">
                            <el-checkbox v-for="chkbox in module.entityList" :class="'col-sm-3'" :label="chkbox.key" :key="chkbox.key">{{chkbox.defaultable == true?chkbox.label+'(推荐)': chkbox.label }}</el-checkbox>
                          </el-checkbox-group>
                      </div>
                    </div>
                  </transition>
                ...
                <div class="form-group" style="text-align:center;">
                  <div class="col-sm-12">
                    ...
                    <button style="background-color: #6db33f;border-color: #1ab394;color: #FFFFFF;" class="btn" @click="postDownload()">生成工程</button>
                  </div>
                </div>
              </div>
            </div>
...
export default {
  ...
  methods: {
    dockerChange: function(data) {
      var that = this;
      debugger;
      if (data == true) {
        that.dockerShow = true;
      } else {
        that.dockerShow = false;
      }
    },
    ...
    postDownload: function() {
      ...
      var that = this;
      that.basicFramework.push(that.scType);
      that.basicFramework.push(that.Independent);
      that.parameter.basePackage = this.modules[1].entityList[3].value;
      that.parameter.dockerTemplate = Boolean(this.dorckerType);
      that.parameter.groupId = this.modules[1].entityList[0].value;
      that.parameter.javaVersion = this.modules[1].entityList[4].value;
      that.parameter.language = this.modules[0].entityList[1].value;
      that.parameter.metadata.appType = "eureka-server";
      that.parameter.metadata.chooseComponent = that.basicFramework;
      that.parameter.pomArtifactId = this.modules[1].entityList[1].value;
      that.parameter.proType = this.modules[0].entityList[0].value;
      that.parameter.projectName = this.modules[1].entityList[2].value;
      that.parameter.springBootVersion = this.modules[0].entityList[2].value;
      console.log(that.parameter);

      const content = "";
      const fileName = "eureka-server";
      document.createElement("a");
      // 非IE下载
      const elink = document.createElement("a");
      // elink.download = fileName;
      elink.style.display = "none";
      elink.href = "http://api-codegen.smartscity.com/download?json=" + encodeURI(JSON.stringify(that.parameter));
